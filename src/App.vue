...
<template>
  <main class="m-auto pt-6">
    <input v-model="query" @keyup.enter="performSearch" class="border shadow-lg w-1/3 m-auto block" placeholder="Search for pages containing [x]..." />
    ...
    <div v-for="result in results" :key="result.id" class="flex border-t border-b p-4">
      ...
      <div class="flex-grow">
        <h1 class="text-2xl">{{ result.title }}</h1>
